...
// reducer function OUTSIDE component function
...
const Login = (props) => {
  ...
  const passwordChangeHandler = (event) => {
    ...
    // comment out. why?
    // not optimal:
    // react schedules state updates in a certain sequence, so
    // it's suboptimal to setState within another handler function
    setFormIsValid(emailState.value.includes("@") && passwordState.isValid);
  };

  const validateEmailHandler = () => {
    console.log("email blurring");
    dispatchEmail({ type: "INPUT_BLUR" });
  };

  const validatePasswordHandler = () => {
    console.log("pwd blurring");
